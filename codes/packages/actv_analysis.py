...
def get_PNs(actv, numbers, sizes, min_nidx, max_nidx, min_sz_idx, max_sz_idx):
    ###
    ## INPUT:
    ...
    avg_actv_nxs_ = avg_actv.reshape(actv.shape[0], 10, 10)
    avg_actv_nxs = avg_actv_nxs_[:,min_nidx:max_nidx,min_sz_idx:max_sz_idx+1]
    PN_by_size = numbers[np.argmax(avg_actv_nxs, axis=1)]
    oPN = numbers[min_nidx:max_nidx+1][np.argmax(np.mean(avg_actv_nxs,axis=2),axis=1)]
    return PN_by_size, oPN


def get_PSs(actv, numbers, sizes, min_nidx, max_nidx, min_sz_idx, max_sz_idx):
    ###
    ## INPUT:
    #   1. actv: activity matrix
    #   2. numbers
...
def get_selective_units(net, relu, epoch, actv_thres=0.05, p_val=0.01, min_num_idx=0, max_num_idx=9,
                        min_sz_idx=3, max_sz_idx=9, numbers=range(2,21,2), inst=500, PN=[], PS=[]):
    """
    ...
    if min_sz_idx != max_sz_idx:
        take = np.arange(0,100).reshape(10,10)[min_num_idx:max_num_idx,min_sz_idx:max_sz_idx+1].reshape(len(numbers)*(max_sz_idx-min_sz_idx+1))
    else:
        take = np.arange(0,100).reshape(10,10)[min_num_idx:max_num_idx,min_sz_idx:max_sz_idx+1].flatten()
    ...
    # Load units
    with open(f'pkl/network{net}_Relu{relu}_epoch{epoch}.pkl', 'rb') as f:
        units = pickle.load(f)
...
def get_random_active_units(net, relu, epoch, actv_thres=0.05, p_val=0.01, min_num_idx=1, max_num_idx=9,
                        min_sz_idx=3, max_sz_idx=9, numbers=range(2,21,2), num_units=200, PN=[], PS=[]):
    """
    The function loads actv_net and unit objects from the corresponding pickle files, 
    and identifies units that have at least actv_thres response rate within the specified 
    size range, and show selectivity to both number and size. 
    ...
    # Subset actv_net based on size range
    if min_sz_idx != max_sz_idx:
        take = np.arange(0,100).reshape(10,10)[min_num_idx:max_num_idx+1,min_sz_idx:max_sz_idx+1].reshape((max_num_idx-min_num_idx+1)*(max_sz_idx-min_sz_idx+1))
    else:
        take = np.arange(0,100).reshape(10,10)[min_num_idx:max_num_idx+1,min_sz_idx:max_sz_idx+1].flatten()

    actv_szAtoB = actv_net[:,take,:]

    # Load units
    with open(f'pkl/4to20/network{net}_Relu{relu}_epoch{epoch}_4to20.pkl', 'rb') as f:
        units = pickle.load(f)

    # select units whose activities corresponding to images will be used for svm training and testing:
    units_active = [units[i].id for i in range(len(units)) if units[i].no_response_subset is not True]

    # Extract response vectors for active, selective units
    response_vectors = actv_szAtoB[units_active, :, :]
    avg_response_vectors = np.mean(response_vectors, axis=2)

    return units_active, avg_response_vectors



def get_avg_response_vectors(net, relu, epoch, units, min_num_idx=1, max_num_idx=9,
    ...
    # Extract response vectors for active, selective units
    response_vectors = actv_szAtoB[units, :, :]
    avg_response_vectors = np.mean(response_vectors, axis=2)

    return avg_response_vectors
...
def update_PNS(units, actv, numbers=np.arange(2,21,2), sizes = np.arange(4,14), min_nidx=0, max_nidx=9, min_sz_idx=3, max_sz_idx=9):
    """
    Update the Preferred Number (PN) and Preferred Size (PS) attributes of the units.

    Parameters:
    units (list): List of unit objects.
    actv (numpy.ndarray): 3D numpy array containing activity of units.
    numbers (numpy.ndarray, optional): Array of numbers used in the experiment. Defaults to np.arange(2,21,2).
    sizes (numpy.ndarray, optional): Array of sizes used in the experiment. Defaults to np.arange(4,14).
    min_sz_idx (int, optional): Minimum index for size. Defaults to 3.
    max_sz_idx (int, optional): Maximum index for size. Defaults to 9.

    Returns:
    list: The updated list of units.
    """
    PNs=get_PNs(actv, numbers, sizes, min_nidx, max_nidx, min_sz_idx, max_sz_idx)[1]
    PSs=get_PSs(actv, numbers, sizes, min_nidx, max_nidx, min_sz_idx, max_sz_idx)[1]
...
def update_response_rate(units, actv, subset=False):
    """
    Update the response rate attribute of the units.

    Parameters:
    units (list): List of unit objects.
    actv (numpy.ndarray): 3D numpy array containing activity of units.
    subset (bool, optional): If True, update the 'response_rate_subset' attribute. Otherwise, update the 'response_rate' attribute.
...
def update_monotonicity(units, actv, min_sz_idx=3, max_sz_idx=9, subset=False):
    """
    Update the num_monotonicity and size_monotonicity attributes of the units.
    ...
    min_sz_idx (int, optional): Minimum index for size. Defaults to 3.
    max_sz_idx (int, optional): Maximum index for size. Defaults to 9.
    subset (bool, optional): If True, calculate monotonicity on subset of data. Otherwise, use the entire data.
...
def update_spearmanr(units, actv, min_nidx=0, max_nidx=9, min_sz_idx=3, max_sz_idx=9, subset=False):
    """
    Update the num_monotonicity and size_monotonicity attributes of the units.
    ...
    if subset:
        avg_actv_nxs_subset = avg_actv_nxs[:,min_nidx:max_nidx, min_sz_idx:max_sz_idx+1]
    else:
        avg_actv_nxs_subset = avg_actv_nxs

    for i, sample in enumerate(avg_actv_nxs_subset):
        # For each sample, initialize lists to store coefficients for rows and columns
        sample_coeffs_rows = []
        sample_coeffs_cols = []

        # Compute Spearman correlations for rows
        for row in sample:
            rho, _ = scipy.stats.spearmanr(row, range(1, len(row) + 1))
            sample_coeffs_rows.append(rho)

        # Transpose the sample to compute correlations for columns
        sample_T = sample.T
        for col in sample_T:
            rho, _ = scipy.stats.spearmanr(col, range(1, len(col) + 1))
            sample_coeffs_cols.append(rho)

        # Assign the computed values to the corresponding unit
        units[i].spearmanr_size = np.mean(sample_coeffs_rows)
        units[i].spearmanr_number = np.mean(sample_coeffs_cols)
