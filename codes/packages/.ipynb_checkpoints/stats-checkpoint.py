...
def pkl_anova2(net, relu, epoch, min_sz_idx=3, max_sz_idx=9, numbers=range(2,21,2), inst=500):
    """
    ...
    if main=='number':
        ...
        for num in numbers:
            for sz in range(min_sz_idx, max_sz_idx+1):
                idx = (num-2)//2 * num_sizes + sz - min_sz_idx
                label = f'{num}-[{sz}]'
                x_labels[idx] = label
                y_labels[idx] = label
        ...
        for sz in sizes:
            for nb in range(min_num_idx, max_num_idx+1):
                idx = (sz-4) * num_numbers + nb - min_num_idx
                label = f'{sz}-[{nb}]'
                x_labels[idx] = label
                y_labels[idx] = label
...
def plot_mds(cos_sim_matrix, relu, epoch, dim=2, labels=None, numbers=range(2,21,2), sizes=range(7,14)):
    """
    ...
    for i, label in enumerate(labels):
        num, sz = [int(x) for x in (label.split('-')[0], label.split('-')[1].strip('[]'))]  # Extract number and size from label
    ...
    ax.set_title('MDS plot of average cosine similarity', fontsize=20)
...
def process_block(block, units, actv_2D, numbers, sizes, inst):
    failed_units = []
    for i, u in enumerate(block):
        try:
            anova2_results = units[u].add_anova2(actv_2D, numbers, sizes, inst, parallel=1)
...
def handle_failed_blocks(failed_sub_blocks, units, actv_2D, numbers, sizes, inst):
        ...
        for sub_block in failed_sub_blocks:
            ...
            for sb_idx, sub_block in enumerate(sub_blocks):
                if len(sub_block) <= 100:
                    ...
                    failed_units = process_block(sub_block, units, actv_2D, numbers, sizes, inst)
    ...
    return failed_sub_blocks, success_blocks



def pkl_anova2_V2(net, relu, epoch, min_sz_idx=3, max_sz_idx=9, numbers=np.arange(2,21,2), inst=500):
    """
    Function: This function performs an ANOVA2 analysis on units in a neural network that have nonzero activations.
    ...
    csv_filename = f'csv/network{net}_Relu{relu}_epoch{epoch}_obj.csv'
    ...
    if len(incomplete_units) ==0: ## if there is nothing to do, simply terminate the function
        ...
        units = actv_analysis.update_PNS(units, actv_net, numbers=numbers, sizes = np.arange(4,14), min_sz_idx=min_sz_idx, max_sz_idx=max_sz_idx)
        ...
        for bk_idx, block in enumerate(blocks):
            ...
            while len(block) > 1:
                ...
                if failed_sub_blocks:
                    ...
                    failed_sub_blocks, success_blocks = handle_failed_blocks(failed_sub_blocks, units, actv_2D, numbers, sizes, inst)
